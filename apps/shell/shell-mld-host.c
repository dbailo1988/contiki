/**
 * \file
 *         Shell library for hosts to test MLD
 *
...
PROCESS(shell_print_host_table_process, "print_host_table");
SHELL_COMMAND(print_host_table_command,
              "print_host_table",
              "print_host_table: print mld host table",
              &shell_print_host_table_process);
...
PROCESS_THREAD(shell_send_done_process, ev, data)
{
    ...
    if (aux != NULL) ctimer_stop(&aux->delay_timer);
    printf("DELAY_TIMER STOPPED, NO REPORT \n");
    mld_done(&group_addr, &source_addr,&preferred_parent);
    erase_listener(&group_addr, &source_addr, &preferred_parent);

    PROCESS_END();
}

PROCESS_THREAD(shell_print_host_table_process, ev, data)
{
    PROCESS_BEGIN();
